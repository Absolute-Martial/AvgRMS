...
def _load_local_investigator() -> Callable[[str], dict[str, Any]]:
    """Load the local investigation entrypoint from the workspace checkout."""
...
def run_local_investigation(prompt: str) -> dict[str, Any]:
    """Execute the workspace-local investigation bridge."""
    return _load_local_investigator()(prompt)


def execute_local_investigation(prompt: str) -> None:
    """Run the local investigation flow without the remote agent service."""
    result = run_local_investigation(prompt)
    summary = result.get("synthesis", {}).get("summary", "Investigation complete.")
    adapter_mode = result.get("adapter_mode", "unknown")

    console.print(
        Panel.fit(
            f"[bold green]Local investigation complete[/bold green]\n\n"
            f"Prompt: {prompt}\n"
            f"Adapter mode: {adapter_mode}",
            title="🦊 SentinentialFox",
        )
    )
...
@click.option(
    "--local-investigate",
    metavar="PROMPT",
    help="Run the local investigation flow directly and exit",
)
@click.option(
    "--agent-url",
    envvar="AGENT_URL",
...
def main(
    agent: str,
    list_agents: bool,
    local_investigate: Optional[str],
    agent_url: str,
    config_url: str,
    token: Optional[str],
):
    ...
    if local_investigate:
        execute_local_investigation(local_investigate)
        return
